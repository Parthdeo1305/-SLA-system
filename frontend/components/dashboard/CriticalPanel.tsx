...
export default function CriticalPanel({ shipments }: CriticalPanelProps) {
  return (
    <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl overflow-hidden flex flex-col h-full shadow-xl">
      <div className="p-4 border-b border-[var(--color-border)] flex items-center justify-between bg-[var(--color-danger-bg-subtle)]">
        <div className="flex items-center gap-2">
          <AlertCircle size={18} className="text-red-500" />
          <h3 className="text-sm font-bold text-[var(--color-text-primary)] uppercase tracking-wider">Critical Shipments</h3>
        </div>
        <span className="text-[10px] bg-red-500/20 text-[var(--color-danger-text)] px-2 py-0.5 rounded-full font-bold uppercase">Immediate Action</span>
      </div>
          ...
          shipments.map((s) => (
            ...
            >
              <div className="flex justify-between items-start mb-2">
                <span className="text-sm font-mono font-bold text-[var(--color-text-primary)] group-hover:text-[var(--color-brand-text)] transition-colors">
                  {s.orderId}
                </span>
              ...
              <div className="grid grid-cols-2 gap-y-2">
                <div className="flex items-center gap-1.5 text-xs text-[var(--color-text-muted)]">
                  <User size={12} className="text-[var(--color-brand-text)]/70" />
                  <span className="truncate">{s.deliveryAgent?.name || 'Unassigned'}</span>
                </div>
                <div className="flex items-center gap-1.5 text-xs text-[var(--color-text-muted)]">
                  <MapPin size={12} className="text-[var(--color-brand-text)]/70" />
              ...
              <div className="mt-3 flex items-center justify-end text-[10px] font-bold text-[var(--color-brand-text)] opacity-0 group-hover:opacity-100 transition-all">
